...
def krmt(request):
    ...
    with MongoClient('mongodb://192.168.0.136:27017') as client:
        db = client.mydb 
        data_list = list(db.mountain2.find({}))
        ...
        for data,data2 in zip(data_list,data_list2):
            ...
            pop_text = folium.Html(
                f"<img src={data['mt_img_path_preview']}><br>" +
                f"{data['mt_name']}<br>" +
...
def acc(request):
    ...
    with MongoClient('mongodb://192.168.0.136:27017') as client:
        ...
        for data,data2 in zip(data_list,data_list2):
            ...
            pop_up = folium.Popup(pop_text)
            # img_path = data['mt_img_path']
            # iframe = folium.IFrame(img_path, width=300, height=300)
            # pop_up = folium.Popup(iframe,pop_text, max_width=650)
            folium.Marker(lat_lon, popup=pop_up, tooltip=data['mt_name']).add_to(m)
